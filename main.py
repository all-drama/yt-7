...
def video(url):
    ...
    if 'selected_stream' not in session_state:
        session_state.selected_stream = st.selectbox("Select Video Resolution", resolutions)
    else:
        session_state.selected_stream = st.selectbox("Select Video Resolution", resolutions, index=resolutions.index(session_state.selected_stream))

    # Find the selected stream based on the resolution string
    selected_stream = resolution_dict.get(session_state.selected_stream)
...
session_state = SessionState.get(selected_stream=None)
